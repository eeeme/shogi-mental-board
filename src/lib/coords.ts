 ...
 * - TTS には符号をそのまま渡さず、ひらがなの読み（例 "ななろく"）に変換する。
 ...
 * 筋・段のどちらの読みにも同じ表を用いる（例: 7六 → "なな" + "ろく"）。
 *
 * 2 と 5 は 1 モーラ（「に」「ご」）で TTS だと音が飲まれて聞き取りにくいため、
 * 2="にぃ" / 5="ごぅ" と伸ばして読ませる（筋・段の両方）。実機で不自然なら
 * 長音符版「にー」「ごー」にフォールバックしてよい。
 */
const NUM_YOMI: Record<YomiStyle, readonly string[]> = {
  modern: ['いち', 'にぃ', 'さん', 'よん', 'ごぅ', 'ろく', 'なな', 'はち', 'きゅう'],
  classic: ['いち', 'にぃ', 'さん', 'し', 'ごぅ', 'ろく', 'しち', 'はち', 'く'],
}
...
/** マスの読み仮名（筋→段の順）。例: {file:7,rank:6} → "ななろく"。 */
